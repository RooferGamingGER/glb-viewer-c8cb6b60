...
/**
 * Measure lengths of segments in a polygon
 * @param points - Array of 3D points
 * @returns Array of segment lengths
 */
export const measureSegmentLengths = (points: Point[]): number[] => {
  ...
  for (let i = 0; i < points.length; i++) {
    const p1 = points[i];
    const p2 = points[(i + 1) % points.length]; // Wrap around to first point for the last segment
...
/**
 * Calculate the center of a polygon (similar to centroid but simpler)
 * @param points - Array of 3D points
 * @returns Point representing the center
 */
