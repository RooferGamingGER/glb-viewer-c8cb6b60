...
/**
 * Creates a PDF document definition with measurement data
 */
const createPdfDocDefinition = async (
...
/**
 * Legacy download method for browsers that don't support File System Access API
 */
const legacyDownload = (blob: Blob, filename: string): Promise<boolean> => {
 ...
 */
const modernDownload = async (blob: Blob, filename: string): Promise<boolean> => {
  try {
...
export const generateMeasurementsPDF = async (
  ...
  try {
    ...
    const docDefinition = await createPdfDocDefinition(measurements, projectData);
    ...
    return new Promise<boolean>((resolve, reject) => {
      try {
        ...
        pdfDocGenerator.getBlob(async (blob) => {
          try {
            // Try using modern download method first
            if ('showSaveFilePicker' in window) {
              try {
                const success = await modernDownload(blob, defaultFilename);
                resolve(success);
              } catch (error) {
                console.warn('Modern download failed, falling back to legacy method:', error);
                const success = await legacyDownload(blob, defaultFilename);
                resolve(success);
              }
            } else {
