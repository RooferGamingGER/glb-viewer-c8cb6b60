  ...
  try {
    ...
    if (areaMeasurements.length > 0) {
      ...
      areaMeasurements.forEach((measurement, index) => {
        ...
        // ========== GENERIC AREA PAGE (always rendered, even for solar) ==========
        ...
        // ========== ADDITIONAL SOLAR PV PAGE (separate from area page) ==========
        if (measurement.pvModuleInfo) {
          ...
          solarPage.style.pageBreakInside = 'avoid';

          // Header
          const solarHeader = document.createElement('div');
          solarHeader.style.borderBottom = '2px solid #3b82f6';
          solarHeader.style.paddingBottom = '10px';
          solarHeader.style.marginBottom = '20px';
          const solarTitle = document.createElement('h2');
          solarTitle.style.margin = '0';
          solarTitle.style.color = '#1e40af';
          solarTitle.textContent = `PV-Fläche ${solarIndex}${measurement.description ? `: ${measurement.description}` : ''}`;
          solarHeader.appendChild(solarTitle);
          solarPage.appendChild(solarHeader);

          // Two-column layout
          const pvContentRow = document.createElement('div');
          pvContentRow.style.display = 'flex';
          pvContentRow.style.gap = '20px';
          pvContentRow.style.marginBottom = '20px';

          // Left: PV layout image
          const pvImageColumn = document.createElement('div');
          pvImageColumn.style.flex = '1';
          pvImageColumn.style.minWidth = '0';
          ...
          if (pvLayoutDataUrl) {
            ...
            pvImageColumn.appendChild(layoutImg);
            ...
            pvImageColumn.appendChild(fallbackImg);
          }
          pvContentRow.appendChild(pvImageColumn);

          // Right: Metrics boxes
          const pvDetailsColumn = document.createElement('div');
          pvDetailsColumn.style.width = '200px';
          pvDetailsColumn.style.flexShrink = '0';
          ...
          if (pvInfo.roofDirection || pvInfo.roofAzimuth !== undefined) {
            metrics.push({ label: 'Ausrichtung', value: `${pvInfo.roofDirection || ''}${pvInfo.roofAzimuth !== undefined ? ` ${pvInfo.roofAzimuth.toFixed(1)}°` : ''}`, bg: '#faf5ff', border: '#a855f7', color: '#7e22ce' });
          }
          ...
          metrics.forEach(m => {
            ...
            pvDetailsColumn.appendChild(box);
          });

          pvContentRow.appendChild(pvDetailsColumn);
          solarPage.appendChild(pvContentRow);
